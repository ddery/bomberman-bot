...
def dekatiMusuh(musuh,mat,jangkauan):
	langkah = mat[musuh["X"]][musuh["Y"]][0]
	if 	langkah == KANAN:
		cek = (musuh["X"]+1,musuh["Y"])
	elif langkah == KIRI:
		cek = (musuh["X"]-1,musuh["Y"])
	elif langkah == ATAS:
		cek = (musuh["X"],musuh["Y"]-1)
	elif langkah == BAWAH:
		cek = (musuh["X"],musuh["Y"]+1)
	else:
		print("exception")
		return cariTempatAman(player,Peta,jangkauan)
	if cek not in jangkauan:
		return langkah
	else:
		print("exception2")
		return cariTempatAman(player,Peta,jangkauan)
...
# print(currentPos)
# print(generatePathUtil(currentPos,(musuh["X"],musuh["Y"]),Peta))
Mat = generateMatriksJarak(currentPos,Peta)
# for i in range(colSize+1):
# 	for j in range(colSize+1):
# 		print(i," ",j," ",Mat[j][i])
# print("Player = ", currentPos[0]," ",currentPos[1])
# print("Musuh = ",musuh["X"]," ",musuh["Y"])
# print(Mat[musuh["X"]][musuh["Y"]])
if currentPos in bombJangkauan:
	print(">> Ada bom")
	move = cariTempatAman(player,Peta,bombJangkauan) 
else:
	if itemBagus and isAdaJalandariPlayer((itemBagus[0][1],itemBagus[0][2]),Mat):
		...
		if isAdaJalandariPlayer((musuh["X"],musuh["Y"]),Mat):
			 ...
			 print(Mat[musuh["X"]][musuh["Y"]])
			 move = dekatiMusuh(musuh,Mat,bombJangkauan)
